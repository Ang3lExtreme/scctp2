...
public class AzureManagement {
	...
	public static final boolean CREATE_REDIS = false;

	// TODO: change your suffix and other names if you want
	static final String MY_SUFFIX = "57609"; // Add your suffix here
	...
	static final String[] AZURE_FUNCTIONS_NAME = Arrays.stream(REGIONS)
			.map(reg -> "scc2223fun" + reg.name() + MY_SUFFIX).toArray(String[]::new);
	...
	public static StorageAccount createStorageAccount(Azure azure, String rgName, String name, Region region) {
		System.out.println(
				"Creating Storage account: name = " + name + " ; group = " + rgName + " ; region = " + region.name());
		StorageAccount storageAccount = azure.storageAccounts().define(name).withRegion(region)
				...
				.withSku(StorageAccountSkuType.STANDARD_LRS).create();
	...
	public static void main(String[] args) {
		try {
			...
			if (args.length == 1 && args[0].equalsIgnoreCase("-delete")) {
				...
				// aux: print app name and resource group name to add when running "mvn azure-webapp:config"
				for (int i = 0; i < REGIONS.length; i++) {
					System.out.println("App name: " + AZURE_APP_NAME[i] + "; Function name: " + AZURE_FUNCTIONS_NAME[i] + "; Resource group: " + AZURE_RG_REGIONS[i]);
				}
				...
				if (CREATE_COSMOSDB) {
					Thread th = new Thread(() -> {
						try {
							...
							createCosmosCollection(cosmosClient, AZURE_COSMOSDB_DATABASE, "auction", "/id", null);
							createCosmosCollection(cosmosClient, AZURE_COSMOSDB_DATABASE, "bid", "/id", null);
							createCosmosCollection(cosmosClient, AZURE_COSMOSDB_DATABASE, "question", "/id", null);
